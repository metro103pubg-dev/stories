...
async def admin_cancel_handler(message: Message):
    ...
    await message.answer("❌ Действие отменено. Возврат в админку:", keyboard=admin_root_kb())
...
async def act_give_coins(message: Message):
    ...
    await message.answer(f"Сколько монет начислить пользователю {uid}? (введите число):", keyboard=cancel_kb())
